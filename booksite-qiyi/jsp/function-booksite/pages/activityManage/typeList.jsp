...
$(document).ready(function(){
	$('#addentity').submit(function(){
		...
		{
			$('#addtypenote').html('<font color=red>输入有误</font>');
			return false;
		}else{
				$('#addtypenote').html('<font color=green>输入正确</font>');
		}
		if(addsort == "" || !isNumber(addsort))
		{
			$('#addsortnote').html('<font color=red>输入有误</font>');
			return false;
		}else{
				$('#addsortnote').html('<font color=green>输入正确</font>');
		}
	});
});
</script>
        ...
        <div class="box_main_center">
            ...
            <s:iterator value="listActivityType" status="sta">  
              <tr bgcolor="#FFFFFF">
                ...
                <td>
                	<a id="activityTypeId" editType='<s:property value="activityType_id"/>' style="cursor: pointer;" class="a_1 edit_jibie2">编辑</a>
