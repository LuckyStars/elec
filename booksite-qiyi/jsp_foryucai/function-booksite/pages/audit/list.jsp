...
function cancelBook(id,state){
	...
	if(confirm(msg)){
		/* if(window!=window.top){ 
			window.top.location.href="${prc}/bookSite/updateAudit.action?id="+id+"&state="+state;
		} */  
		location.href="${prc}/bookSite/updateAudit.action?id="+id+"&state="+state;
		//top.location.hash="${prc}/bookSite/updateAudit.action?id="+id+"&state="+state;
	}
}
</script>
</head>
...
<div class="box">
	<div class="box_head">
        ...
        <font size="2" color="#336699">&raquo</font><a href="${prc}/bookSite/searchBookSite.action">场馆审批</a>
        </div>
    ...
    <div class="box_main">
        ...
        <s:form method="post" namespace="/bookSite" action="listAudit">	
            <b>选择查询条件：</b>
            <select name="state">
              <option value="0"  ${state=='0'?"selected=selected":""} >待审核</option>
              <option value="1"  ${state=='1'?"selected=selected":""}>已通过</option>
              <option value="2"  ${state=='2'?"selected=selected":""}>已完成</option>
              <option value="-1" ${state=='-1'?"selected=selected":""} >已退回</option>
            </select>
            <ul id='hideli'>
            	<li id='1' class="li_mr">
              ...
              <tr bgcolor="#FFFFFF">
                ...
                <td>
                	<s:if test="bookSite_statusId ==0">
	                	...
	                	<a class="a_3" href="javascript:cancelBook('${bookSite_id}',1);" > 通过 </a>
	                	<a class="a_3" href="javascript:cancelBook('${bookSite_id}',3);" > 退回 </a>
                	</s:if><s:else>
                		<a class="a_3" href="${prc}/bookSite/detailBookSite.action?id=${bookSite_id}" > 查看 </a>
                	</s:else>
                </td>
