...
public class ECCourseAction extends ElecBaseAction {
	...
	public void exportExl() {
		String ucGradeId = request.getParameter("ucGradeId");
		...
		List<ECCourseVO> voList = ecCourseBiz.findPage(ecCond, null,ucGradeId);
		...
		if (voList != null && voList.size() > 0) {
			...
			for (ECCourseVO vo : voList) {
				...
				for (int i = 0; i < len; i++) {
					sb.append(
							WeekType.getById(chList.get(i).getWeekIndex())
									.getName() + " ")
							.append(DateUtil.date2Str(chList.get(i)
									.getWeekStartTime(), "HH:mm:ss")
									+ " - ")
							.append(DateUtil.date2Str(chList.get(i)
									.getWeekEndTime(), "HH:mm:ss"));
					if (i != len - 1)
						sb.append("\r\n");
				}
	...
	public String exportExlWithStuInfo(){
		String gradeId = request.getParameter("ucGradeId");///
		...
		if(voList!=null && voList.size()>0){
			...
			for(ECCourseVO vo : voList){
				...
				dataList = elecStuInfoVoBiz.encapeList(elecSignBiz.findByCourseId(vo.getId()),gradeId);
