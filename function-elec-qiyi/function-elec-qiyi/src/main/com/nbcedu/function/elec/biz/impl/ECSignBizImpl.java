...
public class ECSignBizImpl extends ElecBaseBizImpl<ECSign> implements ECSignBiz{
	...
	public List<ECSign> findByCourseId(String courseId, String gradeId) {
		List<ECSign> temp = this.findByCourseId(courseId);
		if(CollectionUtils.isEmpty(temp)){
			return temp;
		}
		List<ECSign> result = new LinkedList<ECSign>();
		if(StringUtils.isNotBlank(gradeId)){
			for (ECSign ecSign : temp) {
				if(isThisGrade(ecSign.getStuId(),gradeId)) {
	...
	private boolean isThisGrade(String stuId,String gradeId){
		final String pid = client.queryUidPid(1, stuId);
		NbcUcClass clazz = client.queryClass(2,new HashMap<String, String>(){{put("studentId", pid);}});
		return clazz!=null && StringUtils.isNotBlank(String.valueOf(clazz.getGradeNum()))
				&&String.valueOf(clazz.getGradeNum()).equalsIgnoreCase(gradeId);
	}
