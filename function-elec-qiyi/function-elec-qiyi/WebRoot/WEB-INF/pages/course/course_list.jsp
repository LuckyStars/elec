...
<div class="con_conent fixed">
    ...
    <h2 style="margin-bottom:0; border-bottom:0;">
		...
		<input type="hidden" name="ecCond.gradeIds" id="gradeIds" value="${ecCond.gradeIds }" />
	...
	<select name="ecCond.gradeIds" style="width:96px;margin-left:400px;margin-top:10px;" id="ucGradeId" >
		<option value="">-分年级导出-</option>
			<c:forEach items="${ucGradeList}" var="ucGrades" >
				<option title="${ucGrades.gradeName}" value="${ucGrades.id}">${ucGrades.gradeName}</option>
		</c:forEach>
	</select>
		<a style="position:relative;" class="d_btn padlr" href="javascript:void(0);">
			<span id="export_excel">导出</span>
