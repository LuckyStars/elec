...
<script type="text/javascript" src="${med }/js/course/course_edit.js"></script>
...
<div class="con_conent fixed" >
    ...
    <div class="text_box">
        <p>
            <label><fmt:message key="i18nTermName" bundle="${bundler}" />：</label>
            &nbsp;${ec.ecTerm.name }
            <input type="hidden" name="ecCond.termId" value="${ec.termId }" />
            <input type="hidden" name="ecCond.id" value="${ec.id }" />
        </p>
        <p>
            <label>课程分类：</label>
        ...
        <script type="text/javascript">
        	$("select[name='ecCond.typeId']").val("${ec.typeId}");
        </script>
        <p>
            <label>课程名称：</label>
			<input name="ecCond.name" type="text" style="width:500px;" value="${ec.name }" maxlength="50"/>
			<span class="redTip" id="ecCond_name_span">*</span>
        </p>
        <p class="chkbox">
            <label style="padding:0;"><%--年级范围--%><fmt:message key="i18nGradeNames" bundle="${bundler}" />：</label>
            <input type="hidden" name="ecCond.gradeIds" value="${ec.gradeIds }" />
            <input type="hidden" name="ecCond.gradeNames" value="${ec.gradeNames }" />
            ...
            <input class="chk" name="ucGrade_ipt" type="checkbox" value="${ucGrade.id }" style="margin:7px 3px 0 10px; " hiddenName="${ucGrade.gradeName }" />
           	...
           	<script type="text/javascript">
	        	$("input[name='ucGrade_ipt']").val('${ec.gradeIds}'.split(","));
	        </script>
        </p>
            <div class="clear"></div>
        <p>
            <label>限定人数：</label>
			<input name="ecCond.maxStudentNum" type="text" value="${ec.maxStudentNum }" maxlength="4" />
			<span class="redTip" id="ecCond_maxStudentNum_span">*</span>
        </p>
        <p>
            <label>任课老师：</label>
            <input type="hidden" name="ecCond.teacherIds" value="${ec.teacherIds }" />
			<input name="ecCond.teacherNames" type="text" value="${ec.teacherNames }" style="width: 272px;<c:if test="${curUser.courseTeacherViewable }">background:#D4D0C8;</c:if>" 
				title="${ec.teacherNames }" readonly="readonly" />
			<%-- 排除：任课教师自己 --%>
			<c:if test="${curUser.courseTeacherViewable }">
			<input type="button" value="添加" class="add_course_btn" style="background:#D4D0C8; color:#808080; border:1px solid #A0A09D;" />
			</c:if>
			<c:if test="${!curUser.courseTeacherViewable }">
        ...
        <p>
            <label>是否俱乐部课程：</label>
            <input type="hidden" name="ecCond.clubState" value="${ec.clubState }" />
			<input type="checkbox" id="isClubState" />
			<span class="redTip" id="ecCond_clubState_span"></span>
			<script type="text/javascript">
				<c:if test="${ec.clubState==1}">
	        	$("#isClubState").attr("checked", "checked");
				</c:if>
	        </script>
        </p>
        </c:if>
        ...
        <p>
			...
			<script type="text/javascript">
	        	$("select[name='ecCond.courseAttr']").val("${ec.courseAttr}");
	        </script>
        </p>
        </c:if>
        <p>
            <label>报名开始：</label>
			<input name="ecCond.signStartDateStr" value="<fmt:formatDate value="${ec.signStartDate}" pattern="yyyy-MM-dd HH:mm:ss"/>" type="text" class="Wdate" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', maxDate:'#F{$dp.$D(\'d4312\')}', minDate:'<fmt:formatDate value="${currentTerm.openDateStart}" pattern="yyyy-MM-dd HH:mm:ss"/>'})" id="d4311" style="width:135px;" />
            <label>报名结束：</label>
            <input name="ecCond.signEndDateStr" value="<fmt:formatDate value="${ec.signEndDate}" pattern="yyyy-MM-dd HH:mm:ss"/>" type="text" class="Wdate" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',  maxDate:'<fmt:formatDate value="${currentTerm.openDateEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>', minDate:'#F{$dp.$D(\'d4311\')}'})" id="d4312" style="width:135px;" />
            <span class="redTip" id="ecCond_signDate_span">*</span>
        </p>
        <p>
            <label>开课日期：</label>
			<input name="ecCond.classStartDateStr" value="<fmt:formatDate value="${ec.classStartDate}" pattern="yyyy-MM-dd"/>" type="text" class="Wdate" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'d4314\')}', minDate:'#F{$dp.$D(\'d4312\')}'})" id="d4313" style="width:135px;" />
            <label>结课日期：</label>
            <input name="ecCond.classEndDateStr" value="<fmt:formatDate value="${ec.classEndDate}" pattern="yyyy-MM-dd"/>" type="text" class="Wdate" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'#F{$dp.$D(\'d4313\')}'})" id="d4314" style="width:135px;" />
            <span class="redTip" id="ecCond_classDate_span">*</span>
        </p>
        <c:if test="${signType=='shijia'}">
         <p>
            <label>要求课时：</label>
			<input name="ecCond.classhourRequire" type="text" value="${ec.classhourRequire }" maxlength="4" /><span>课时/周</span>
			<span class="redTip" id="ecCond_classhourRequire_span">*</span>
        </p>
        </c:if>
        <%-- 
        <div class="ptextl" style="display:none">
            ...
            <script type="text/javascript">
				$("select[name='ecCond.classhourNum']").val("${ec.classhourNum}");
	        </script>
        </div>
        <div class="ptext fixed" id="classHour_div_parent" style="display:none" >
            ...
            <script type="text/javascript">
				<c:forEach items="${ec.classhourList}" var="ch" varStatus="stats" >
				$("#classHour_div${stats.index+1}").show();
				$("#classHour_div${stats.index+1}").find("select[name='ecCond_weekIndexs']").val("${ch.weekIndex }");
				$("#classHour_div${stats.index+1}").find("input[name='ecCond_weekStartTimes']").val('<fmt:formatDate value="${ch.weekStartTime}" pattern="HH:mm:ss"/>');
				$("#classHour_div${stats.index+1}").find("input[name='ecCond_weekEndTimes']").val('<fmt:formatDate value="${ch.weekEndTime}" pattern="HH:mm:ss"/>');
				</c:forEach>
	        </script>
        </div>
        <div style="display: none;" id="real_classHour_div">
        	<c:forEach items="${ec.classhourList}" var="ch" >
	        	<input type="hidden" name="ecCond_real_weekIndexs" value="${ch.weekIndex }" />
				<input type="hidden" name="ecCond_real_weekStartTimes" value="<fmt:formatDate value="${ch.weekStartTime}" pattern="HH:mm:ss"/>" />
				<input type="hidden" name="ecCond_real_weekEndTimes" value="<fmt:formatDate value="${ch.weekEndTime}" pattern="HH:mm:ss"/>" />
			</c:forEach>
        </div>
        --%>
        <p>
			...
			<select class="sel" name="ecCond.startPlaceId">
				<option value="">--请选择--</option>
				<c:forEach items="${startPlaceList}" var="place" >
	            <option value="${place.id }">${place.name }</option>
				</c:forEach>
			</select>
			<span class="redTip" id="ecCond_startPlaceId_span">*</span>
			<script type="text/javascript">
				$("select[name='ecCond.startPlaceId']").val("${ec.startPlace.id}");
	        </script>
        </p>
		<%--
        <p>
			...
			<script type="text/javascript">
				$("select[name='ecCond.endPlaceId']").val("${ec.endPlace.id}");
	        </script>
        </p>--%>
        <p style="float:left; clear:both          ; height: auto; margin-top: 5px;"> <span class="fl">选课提示：</span>
			<textarea class="fl" name="ecCond.courseComment" maxlength="100" style="height:50px;">${ec.courseComment }</textarea><br/>
        </p>
		...
		<div style="display: none; float:left;" id="detailContent">
	        <p style="float:left; clear:both; height: auto; margin-top: 0px;"> <span class="fl">课程内容：</span>
				<textarea class="fl" name="ecCond.courseContent" style="width:650px;" >${ec.courseContent }</textarea>
	        </p>
	        
	        <p style="float:left; clear:both; height: auto; margin-top: 10px;"> <span class="fl">学生要求：</span>
				<textarea class="fl" name="ecCond.courseRequire" style="width:650px;">${ec.courseRequire }</textarea>
	        </p>
	        
	        <p style="float:left; clear:both; height: auto; margin-top: 10px;"> <span class="fl">备　　注：</span>
				<textarea class="fl" name="ecCond.courseNote" style="width:650px;" >${ec.courseNote }</textarea>
	        </p>
        </div>
        <div class="clear"></div>
    </div>
