$(function(){
	...
	//----------------------- 各种ajax验证 ---------------------
	var ecId = $("input[name='ecCond.id']").val();
	...
	//课程名称
	function checkNameSame(ecCond_name, termId, ecId){
		if(ecCond_name != ""){
			$.getJSON("checkNameEcCourse.action", {name:ecCond_name, termId:termId, courseID:ecId, time:new Date().getTime()}, function(callData){
	...
	$("input[name='ecCond.name']").blur(function(){
		...
		checkNameSame(ecCond_name, termId, ecId);
	});
	
	//限定人数
	function checkMaxStudentNum(maxStudentNum){
		if(maxStudentNum != ""){
			if(!isNegative(maxStudentNum) && isInt(maxStudentNum)){
				$("#ecCond_maxStudentNum_span").html("*");
				return true;
			}else{
				$("#ecCond_maxStudentNum_span").html("限定人数只能为正整数！");
				return false;
			}
		}
	}
	...
	$("#add_course_submit").click(function(){
		...
		$.ajax({
			...
			data: "name="+ ecCond_name +"&termId="+ termId +"&courseID="+ ecId +"&time="+new Date().getTime(),
            async: false,
            dataType: "json",
		...
		var termId = $("input[name='ecCond.termId']").val();
		var checkClasshourBool = true;
		$.ajax({
            type: "POST",
            url: 'checkClasshourEcCourse.action',
			data: ajaxParams +"&courseID=" + ecId +"&teacherNames="+teacherNames + "&teacherIds="+teacherIds+"&startPlaceId="+startPlaceId+"&termId="+termId+"&classStartDateStr="+classStartDateStr+"&classEndDateStr="+classEndDateStr+"&time="+new Date().getTime(),
            async: false,
            dataType: "json",
            success: function(callData){
