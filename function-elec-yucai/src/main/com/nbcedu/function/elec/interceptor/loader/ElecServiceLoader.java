	...
	public Object load(Map<?, ?> param) {
		...
		try {
			...
			if (uid.equals(Constants.ADMIN_UID)) {
				...
				isParent =person.getType().equals(Constants.PERSON_TYPE_PARENT)||person.getType().equals(Constants.PERSON_TYPE_STUDENT);
				
				if (logger.isInfoEnabled()) {
					logger.info("=========elec : person.getName()= " + person.getName());
					logger.info("=========elec :person.getType()= " + person.getType());
					logger.info("=========elec :isParent= " + isParent);
				}
				
				if(userPrivilege==null){//无此用户权限
					if (logger.isInfoEnabled()) {
						logger.info("=========elec : " + "userPrivilege==null");
					}
					if(isParent){
						...
						if (logger.isInfoEnabled()) {
							logger.info("=========elec : " + "getUid" + userPrivilege.getUid());
							logger.info("=========elec : " + "getPid" + userPrivilege.getPid());
							logger.info("=========elec : " + "getPid" + userPrivilege.getUserName());
						}
					}else{//可能：班主任、查看权限教师
						userPrivilege = new ECUserPrivilege();
						userPrivilege.setUid(uid);
						...
						if (logger.isInfoEnabled()) {
							logger.info("=========elec : " + "getUid bean" + userPrivilege.getUid());
							logger.info("=========elec : " + "getPid bean" + userPrivilege.getPid());
							logger.info("=========elec : " + "getPid bean" + userPrivilege.getUserName());
						}
